...
def cal_Pbn_entropy(model,hooks,val_loader):
	...
	for data in tqdm(val_loader):
		...
		for key in hooks.keys():       
			...
			Gamma = torch.std(hooks[key].output, dim=[0,1,2])
...
def main():
	...
	if args.dataset == 'TinyInet':
		...
		milestones=[80,120]
		batch_size = 128
		DATA_DIR = args.DATA_DIR              # data location
		...
		milestones=[24]
		batch_size =16		
		data_path = args.DATA_DIR        # data location
